...
def plot_ceRNA_exp(miRNA_exp: pd.Series,
    ...
    plt.tick_params('both', color='w', labelcolor='w')
    plt.title(f'{mRNA_exp.name}_{miRNA_exp.name}_{circRNA_exp.name}\nr={r}, p={p}')
    plt.legend()
    plt.savefig(f"{out_path}ceRNA_exp_plot/"
                f"{mRNA_exp.name}_{miRNA_exp.name}_{circRNA_exp.name.replace(':', '_').replace('|', '_')}.pdf",
                bbox_inches='tight')
    plt.clf()
...
def main(mRNA_exp_file: str,
    ...
    if not os.path.exists(f'{out_path}ceRNA_exp_plot'):
        os.mkdir(f'{out_path}ceRNA_exp_plot')
...
@click.option('-mrna', help='Input mRNA expression file. (Supported formats: txt, xls, xlsx and csv)')
@click.option('-circ', help='Input circRNA expression file. (Supported formats: txt, xls, xlsx and csv)')
@click.option('-cerna', help='Input ceRNA file as followed content:'
                          '\nmiRNA156\\tChr01:100|1000\\tcircRNA\\n\nmiRNA156\\tPotri.001G001000.1\\tmRNA\\n')
@click.option('-mirna', help='[optional] Input miRNA expression file. (Supported formats: txt, xls, xlsx and csv)')
@click.option('-out', default='./', help='[optional] Output path. {default=./}')
...
def run(mrna, circ, cerna, mirna, out, r1, p1, r2, p2):
    """Identify the expression correlation between ceRNA (mRNA-circRNA) with Pearson coefficient."""
    main(mrna, circ, cerna, mirna, out, r1, p1, r2, p2)
