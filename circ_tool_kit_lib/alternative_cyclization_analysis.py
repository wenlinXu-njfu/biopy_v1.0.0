...
def main(circ_bed_file: str, alt_cyc_min: int, out_file: str, circ_seq_fasta_file: str = None):
    ...
    unique_circ.sort(key=lambda i: (re.findall(r'[a-zA-Z]+', i.split(':')[0])[0],
                                    int(re.findall(r'\d+', i.split(':')[0])[0]),
                                    int(re.findall(r'\d+', i.split(':')[1])[0])))
    content = ''
    for j in unique_circ:
        if circ_list.count(j) >= alt_cyc_min:
            if raw_circ_type_dict:
                ...
                content += f'{j}\t{circ_list.count(j)}\t{circ_type}\n'
            else:
                content += f'{j}\t{circ_list.count(j)}\n'
    if out_file:
...
@click.option('-bed', help='Input circRNA BED file. (Chr_num\\tStart\\tEnd\\tCirc_ID\\tFrame\\tStrand\\tEtc)')
@click.option('-fa', help='[optional] CircRNA sequence FASTA file generated by CircToolKit subcommand "get_circ_seq" '
                          'to annotate the type of circRNA.')
@click.option('-n', type=int, default=2, help='[optional] Output sites which the number of alternative cyclization '
                                              'site exceeds specified value. {default=2}')
@click.option('-out', help='[optional] Output file, if not specified, print results to terminal as stdout.')
def run(bed, fa, n, out):
