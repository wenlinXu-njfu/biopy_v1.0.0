...
def main(genome_fasta_file, genome_gtf_file, circRNA_bed_file, out_file):
    ...
    for line in gtf_file.parse_GTF():
        if line[2] == 'gene':
    ...
    content = ''
    for nucl in Fasta(genome_fasta_file).FASTA_generator():
        try:
...
@click.option('-gtf', help='Input genome GTF file.')
@click.option('-bed', help='Input circRNA locus BED file, the site of first nucleotide is one. '
