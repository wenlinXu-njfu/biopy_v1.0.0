            ...
            if split[2] == 'transcript':
                ...
                if chr_num in gff_dict:
                    ...
                    for gene in gene_list:
                        ...
                        if dis is not None:
                            if out_file:
                                content += f"{chr_num}\t{transcript_id}\t{gene['id']}\t{dis}\t{loc}\t{strand}\n"
                            else:
...
@click.option('-gtf', help='LncRNA annotation file (format: GTF)')
@click.option('-gff', help='Reference annotation file (format: GFF)')
@click.option('-feature', default='mRNA', help='Feature type. {default=mRNA}')
@click.option('-range', 'r', help='Genes within a certain range of upstream and downstream of lncRNA were selected as '
                                  'target genes. {default=100 unit:kb}', type=int, default=100)
@click.option('-out', help='[optional] Output file, if not specified, print results to terminal as stdout.')
def run(gtf, gff, feature, r, out):
    """
    Target genes were predicted according to the location relationship between lncRNA and genes.
    """
    lncRNA_target_gene_prediction(gtf, gff, feature, r, out)
