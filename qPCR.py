...
def main(in_file: str, ref_gene_name: str, control_sample_name: str, out_file: str, figsize: Tuple[float, float]):
    ...
    df = pd.read_excel(in_file, 'Results', 39, usecols=['Sample Name', 'Target Name', 'CT'])
    ...
    writer = pd.ExcelWriter(f'{out_file}.xlsx')
    target_gene_df.to_excel(writer, float_format="%.3f", index=False)
    writer.save()
...
@click.option('-in', 'in_file', help='Input qPCR result excel file.')
@click.option('-ref',
              help='Specify reference gene name. Multiple reference genes are separated by commas. (eg. 18s,ef1)')
@click.option('-control', help='Specify name of control sample.')
@click.option('-figsize', default='10x10', help='[optional] Specify figure size. {default=10x10}')
@click.option('-out', help='Prefix of output file.')
def run(in_file, ref, control, figsize, out):
    """Calculate relative expression based on qPCR results."""
    warnings.filterwarnings("ignore")
    figsize = tuple([float(i) for i in figsize.split('x')])
    main(in_file, ref, control, out, figsize)
